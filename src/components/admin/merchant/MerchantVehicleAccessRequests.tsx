...
interface MerchantVehicleAccessRequestsProps {
  vehicleAccessRequests: any[];
}

export const MerchantVehicleAccessRequests: React.FC<MerchantVehicleAccessRequestsProps> = ({ vehicleAccessRequests }) => {
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold text-slate-800 flex items-center gap-2">
          <Car className="h-5 w-5" />
          Машины нэвтрэх хүсэлт
        </h2>
        <div className="flex items-center gap-2 text-sm text-slate-500">
          <History className="h-4 w-4" />
          <span>Нийт {vehicleAccessRequests?.length || 0} хүсэлт</span>
        </div>
      </div>
      {vehicleAccessRequests && vehicleAccessRequests.length > 0 ? (
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-200">
              {vehicleAccessRequests.map((request: any) => (
