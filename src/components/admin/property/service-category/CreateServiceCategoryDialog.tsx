...
type CreateServiceCategoryDialogProps = {
  trigger: React.ReactNode;
  onCreated?: () => void;
};

const CreateServiceCategoryDialog: React.FC<CreateServiceCategoryDialogProps> = ({ trigger, onCreated }) => {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Талбайн ангилал нэмэх</DialogTitle>
          <DialogDescription>Нэр, код, тайлбарыг оруулаад хадгална уу.</DialogDescription>
        </DialogHeader>
        <form className="space-y-4" onSubmit={handleSubmit}>
          <div className="space-y-2">
            <label className="text-sm font-medium text-slate-700">Нэр*</label>
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Жишээ: Үйлчилгээ" />
          </div>
          <div className="space-y-2">
