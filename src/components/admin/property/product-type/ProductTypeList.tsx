...
import { deleteProductType, getProductTypes } from "@/lib/api";
...
const ProductTypeList: React.FC = () => {
  ...
  const fetchTypes = async (page: number = currentPage, searchText: string = search) => {
    try {
      setLoading(true);
      setError(null);
      const res = await getProductTypes(page, PER_PAGE, "id", "desc", searchText.trim());
  ...
  const handleDelete = async (id: number | string) => {
    const confirmed = window.confirm("Энэ бүтээгдэхүүний төрлийг устгах уу?");
    if (!confirmed) return;
    setDeletingId(id);
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Бүтээгдэхүүний төрөл</h1>
        </div>
